...
## Objective
Verify that required parameter values (fire ratings, occupancy, other rules
the firm declares) are present, consistent, and semantically coherent — the
check Revit cannot do, because "4 HR", "4hr", and "240 min" are four strings
to Revit and one rating to a code official. Draft parameter update CRs where
the correct value is defensible; deliver diagnosis where it is a design
decision. Never guess a rating into existence.
...
1. For each rule in compliance.md, resolve its scope to concrete elements
   and check it. Normalize equivalent values before comparing ("2 HR"
   satisfies "2 hours"; "45 min" satisfies "0.75 hr") — report the value as
...
2. Consistency checks run even without a rules file:
   ...
   - Conflicts: a door rated below its host wall's requirement; instances
     of one type carrying different values.
   - Required parameter empty on an element whose peers all carry a value.
3. Skip Exceptions-log matches. Mark diagnose-only findings NEW or STANDING
   (first-seen date) so repeats read as unresolved, not rediscovered.

## Fix policy: suggest-with-options; diagnose where ambiguous
...
- **Conflict resolution with a defensible answer**: align diverging
  instances to the type's established value, source stated; at most 2
  options when two readings are viable.

Stay diagnose-only (report entry, no CR) when the fix would *invent* a
value — a missing rating, or a door genuinely under-rated for its host
wall: those are design decisions. The entry still names the element, the
...
- Ambiguous or conflicting rules: report the ambiguity itself; enforce
  neither reading.

## Change request format
- Element/type ID, parameter, current value → proposed value
- One-line rationale citing rule and source ("per compliance.md: ratings
  written in hours; '120 min' → '2 hours', IBC 713.4")
- Batch by rule, max 15 updates per CR
- Diagnose-only findings lead the daily report (severity class 1), [HIGH]
  before [MED]
...
sharing a pattern → suggest the compliance.md edit in the daily report;
never silently rewrite the standards file.

## Hard limits
- Parameter writes only, and only values defensible from the rules file or
  the model — never invent a rating. Missing values are always
  diagnose-only.
- Max 2 CRs per day; max 15 diagnose findings per report, overflow to the
  deferred backlog, life-safety first.
