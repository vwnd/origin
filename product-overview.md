...
_Automated daily maintenance for Revit models. "Dependabot for Revit."_
...
owns. _Knowing_ is solved. _Doing_ is not. Every existing tool in this
...
one enjoys. Deferred, it also gets _more_ expensive — a naming
...
That is a positive signal for "closing the loop". Issue detection (beyond the semantics catch) was not difficult. Handing a fix _back_ was (beyond the simple "delete instances" in Revit warning window). Without a return path the ceiling on any
...
human. _A report without a recommendation is just a warning, and Revit
already has warnings._
...
_this firm_ over time rather than staying generic forever.
...
**Fix policy — how much the agent decides:** _auto-propose_ (deterministic
fix, agent adds rationale) · _suggest-with-options_ (judgment fix, stated
preference, max 2) · _diagnose-only_ (often a design condition in progress,
...
dominant convention → "this project has none; define one." _The fleet is
allowed to say it doesn't know._
...
task's Inputs section _is_ that query contract. Principle: **automate before
...
Exceptions log and the standards files — an executable record of what _this
firm_ actually means, assembled as a byproduct of five minutes a day. It
